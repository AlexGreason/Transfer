...
from transfer_utils.triples_utils import write_triples
from cgol_utils.paths import cgolroot
from cgol_utils.pattern_utils import size, density
from cgol_utils.utils import cost, getpop, escapenewlines, trueSLs, get_date_string, expensive_stills, min_paths
...
def get_result(taskqueue, resqueue, vals, inputids, reported, singlereport):
    while True:
        ...
        newres = try_get(resqueue, delay=0.2)
        if newres:
            return newres
    ...
    with open(outfile, 'a') as g, open(costsfile, 'a') as costsf, open(improvefile, 'a') as improvef:
        ...
        while True:
            component, maxcost, depth, _, target = get_result(taskqueue, resqueue, vals, inputids, reported,
                                                              singlereport)
            if not allstarted and id > nthreads:
                # I don't know why, but it's hanging when there's fewer targets than threads unless warmed up like this
                [w.start() for w in workers[1:]]
                allstarted = True
...
def run():
    ...
    # stills += expensive_stills(min_paths, cells=15, cost=11, force_true=True)
    ...
    # stills += ['xs58_0ckgow696wogkcz6aiq2qa404aq2qia6zy223032']
    targetcosts = {}
    for s in stills:
